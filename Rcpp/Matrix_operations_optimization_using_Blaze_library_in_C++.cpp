...
// [[Rcpp::export]]
NumericMatrix mat_mult(NumericMatrix A, NumericMatrix B){   //fastest matrix multiplication algo. so far
...
#tic()
#N <- mat.mult(A,transpose(B))
#Sys.sleep(1)
#toc()

microbenchmark(A%*%B, matrix_multiply(A,B), matrix_multiply1(A,B), matrix_multiply2(A,B), mat.mult(A,B), rcpp_mat_mult(A,B), mat_mult(A,B))
microbenchmark(t(A), transpose(A), trans(A))
microbenchmark(matrix_add(A,B), matrix_add1(A,B))
microbenchmark(matrix_scalar_multiply(50,A), matrix_scalar_multiply1(50,A))
microbenchmark(A%*%B%*%C, matrix_multiply(matrix_multiply(A,B),C), matrix_3_multiply1(A,B,C), matrix_3_multiply2(A,B,C), mat_mult(mat_mult(A,B),C))
*/